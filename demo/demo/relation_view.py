...
def search_entity(request):
    ...
    if (request.GET):
        ...
        if len(entityRelation) == 0:
            ...
            return render(request, 'entity.html', {'ctx': json.dumps(ctx, ensure_ascii=False), 'entity': entity})
        else:
            # 返回查询结果
            ...
            return render(request, 'entity.html',
                          {'entityRelation': json.dumps(entityRelation, ensure_ascii=False), 'entity': entity})
...
def search_relation(request):
    ...
    if (request.GET):
        ...
        returnResult = {'entity1_text': entity1, 'entity2_text': entity2, 'relation_name_text': relation}
        ...
        if (len(entity1) != 0 and len(relation) == 0 and len(entity2) == 0):
            ...
            if (len(searchResult) > 0):
                returnResult['searchResult'] = json.dumps(searchResult, ensure_ascii=False)
                return render(request, 'relation.html', returnResult)
